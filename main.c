...
void moveMotor(double iterations);

void delay(void);

void delaySpeaker(void);
...
int main(void){
	...
	while(1)
	{
		...
		if (seconds > 59)
			seconds = 59;
		double i = 0;
		double iters = 512;		//512 is number of cycles to complete one revolution.
		iters *= ((double)seconds/(double)60);
...
int initMotor()
{
	...
	GPIOA->ODR &= 0xFFFFFFF1; // set GPIOA 1-3 all to 0 initially
	return 0;
}
