...
void LCD_PIN_Init(void){
	...
	RCC->AHB2ENR |= RCC_AHB2ENR_GPIOAEN | RCC_AHB2ENR_GPIOBEN | RCC_AHB2ENR_GPIOCEN | RCC_AHB2ENR_GPIODEN;	//enable port clocks
	
	GPIOA->MODER &= 0x3FC00FFFU;
	GPIOA->MODER |= 0x802AA000U;   //set A pins to AF(10)
	GPIOA->AFR[0] |= 0xBB000000U;	//configure AFR
	GPIOA->AFR[1] |= 0xB0000BBBU;
	
	GPIOB->MODER &= 0x00F3F0F0U;
	GPIOB->MODER |= 0xAA080A0AU;	//set B pins to AF(10)
	GPIOB->AFR[0] |= 0x00BB00BBU;	//configure AFR
	GPIOB->AFR[1] |= 0xBBBB00B0U;
	
	GPIOC->MODER &= 0xFFFC003FU;
	GPIOC->MODER |= 0x0082AA80U;	//set C pins to AF(10)
	GPIOC->AFR[0] |= 0xBBBBB00U; //configure AFR
	GPIOC->AFR[1] |= 0x000000BU;
	
	GPIOD->MODER &= 0x0000FFFFU;
	GPIOD->MODER |= 0xAAAA0000U;	//set D pins to AF(10)
	GPIOD->AFR[0] |= 0x00000000U; //configure AFR
	GPIOD->AFR[1] |= 0xBBBBBBBBU;
...
void LCD_DisplayName(void){
	...
	for(int i=0; i < 8; i++)
	{
		LCD->RAM[i] &= 0x00000000U;	//reset all bits in case something was there before
	}
	LCD->RAM[0] |= 0x84428310U;  	//set bits 4, 22, 15, 31, 9, 17, 8, 26
	
	LCD->RAM[2] |= 0xC480C308U;		//set bits 3, 23, 14, 15, 30, 31, 9, 8, 26

	LCD->RAM[4] |= 0x00000048U;		//set bits 3, 6
	
	LCD->RAM[5] |= 0x00000008U;		//set bits 3
	
	LCD->RAM[6] |= 0x01001010U;		//set bits 4, 12, 24 
	
	LCD->SR |= LCD_SR_UDR;				//request data transfer
}
...
void LCD_Configure(void){
	LCD->CR &= ~(LCD_CR_BIAS);
	LCD->CR |= LCD_CR_BIAS_1;										//set bias to 1/3 (10)
	
	LCD->CR &= ~(LCD_CR_DUTY);
	LCD->CR |= LCD_CR_DUTY_0 | LCD_CR_DUTY_1; 	//set duty to 1/4 (011)
	
	LCD->FCR &= ~(LCD_FCR_CC);									//configure cc in lcd fcr
	LCD->FCR |= LCD_FCR_CC;
	
	LCD->FCR &= ~(LCD_FCR_PON);									//configure PON in lcd fcr
	LCD->FCR |= LCD_FCR_PON;
	
	LCD->CR &= ~(LCD_CR_MUX_SEG);  							//disable mux seg
	
	LCD->CR &= ~LCD_CR_VSEL; 										//set as internal voltage source
	
	while((LCD->SR & LCD_SR_FCRSR) == 0);				//wait until fcrsr is set
	
	LCD->CR |= LCD_CR_LCDEN;										//enable lcd
	
	while((LCD->SR & LCD_SR_ENS) == 0);					//wait until ens is set
	
	while((LCD->SR & LCD_SR_RDY) == 0);					//wait until fcrsr is set
	
	LCD->SR |= LCD_SR_UDR;											//update display request
	
}


void LCD_Clear(void){
 while((LCD->SR & LCD_SR_UDR) != 0);					//wait until off-screen buffer is unlocked
	
	for(int i = 0;i <= 8; i++)
		LCD->RAM[i] = 0;													//clear buffer
	
	//request to transfer datat from off-screen buffer to on-screen buffer
	LCD->SR |= LCD_SR_UDR;
}
...
void LCD_DisplayString(uint8_t* ptr){
	...
	{
		LCD_WriteChar(&ptr[i], 0, 0, i);					//Loop until null to write each character 
	}
	
	LCD->SR |= LCD_SR_UDR;											//request data transfer to LCD
}
